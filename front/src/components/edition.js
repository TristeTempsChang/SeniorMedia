...
function Edit() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/show_recipe` , { params: { _id: recipeId } })
      .then((response) => {
        ...
        setFormValues({
          ...
          img: recipeData.img,
          contenu: recipeData.contenu,
          time: recipeData.time,
          view: recipeData.view,
        });
      })
      .catch((error) => {
        console.error(error);
  ...
  const handleFormSubmit = () => {
    ...
    axios
      .put(`http://localhost:3001/update_recipe`, {
        _id: recipeId,
        nom,
        type,
        img,
    ...
    <div>
      ...
      <div>
        <div className='form'>
          ...
          <br />
          <br />
          <ColorButton variant="contained" className='espace' onClick={handleFormSubmit}><SaveAltIcon className='add' />Enregistrer les changements</ColorButton>
        </div>
      </div>
      <Footer />
    </div>
